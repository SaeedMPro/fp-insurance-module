...
let claim1Id, claim2Id, claim3Id
...
await step('API: admin + employee tokens', async () => {
  ...
  employeeToken = (await apiLogin('sara.ahmadi', 'Employee123!')).token
})
...
await step('API: reset demo-account roles (self-healing precondition)', async () => {
  ...
  const want = { 'sara.ahmadi': 'employee', 'reza.karimi': 'employee', reviewer: 'reviewer', auditor: 'auditor' }
...
await step('UI: employee creates & submits an عینک claim', async () => {
  ...
  await uiLogin(page, 'sara.ahmadi', 'Employee123!')
  ...
  await fillByLabel(page, 'تاریخ فاکتور', todayInput())
...
await step('UI: admin publishes a new عینک rule version (no code change)', async () => {
  ...
  await fillByLabel(page, 'تاریخ اعمال از', todayInput())
